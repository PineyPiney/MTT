...
class WeaponType(override val id: String, val damageType: DamageType, val numDice: Int, val sides: Int, val martial: Boolean, val ranged: Boolean, val finesse: Boolean, val heavy: Boolean, val light: Boolean, val reach: Boolean, val twoHanded: Boolean, val versatile: Boolean, val nearDistance: Float, val farDistance: Float) : EquipmentType {
	...
	companion object {
		...
		fun ranged(id: String, sides: Int, martial: Boolean, nearDistance: Float, farDistance: Float, finesse: Boolean = false, heavy: Boolean = false, light: Boolean = false, twoHanded: Boolean = false): WeaponType {
			val type = WeaponType(id, DamageType.Companion.PIERCING, 1, sides, martial, true, finesse, heavy, light, false, twoHanded, false, nearDistance, farDistance)
			return type.also { set.add(it) }
		}

		// Simple Melee
		val CLUB = melee("club", DamageType.Companion.BLUDGEONING, 1, 4, false, light = true)
		val GREAT_CLUB = melee("great_club", DamageType.Companion.BLUDGEONING, 1, 8, false, twoHanded = true)
		val DAGGER = melee("dagger", DamageType.Companion.PIERCING, 1, 4, false, finesse = true, light = true, nearDistance = 4f, farDistance = 12f)
		val HAND_AXE = melee("hand_axe", DamageType.Companion.SLASHING, 1, 6, false, light = true, nearDistance = 4f, farDistance = 12f)
		val JAVELIN = melee("javelin", DamageType.Companion.PIERCING, 1, 6, false, nearDistance = 6f, farDistance = 24f)
		val LIGHT_HAMMER = melee("light_hammer", DamageType.Companion.BLUDGEONING, 1, 4, false, light = true, nearDistance = 4f, farDistance = 12f)
		val MACE = melee("mace", DamageType.Companion.BLUDGEONING, 1, 6, false)
		val QUARTER_STAFF = melee("quarter_staff", DamageType.Companion.BLUDGEONING, 1, 6, false, versatile = true)
		val SICKLE = melee("sickle", DamageType.Companion.SLASHING, 1, 4, false, light = true)
		val SPEAR = melee("spear", DamageType.Companion.PIERCING, 1, 6, false, versatile = true, nearDistance = 4f, farDistance = 12f)

		// Martial Melee
		val SHORT_SWORD = melee("short_sword", DamageType.Companion.PIERCING, 1, 6, finesse = true, light = true)
		val LONG_SWORD = melee("long_sword", DamageType.Companion.SLASHING, 1, 8, versatile = true)
		val GREAT_SWORD = melee("great_sword", DamageType.Companion.SLASHING, 2, 6, heavy = true, twoHanded = true)
