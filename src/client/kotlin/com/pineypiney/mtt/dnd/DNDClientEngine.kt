...
class DNDClientEngine(val client: MinecraftClient) : DNDEngine() {
	...
	override val players: List<DNDPlayerEntity> get() = client.player?.let { it.clientWorld?.getEntitiesByType(MTTEntities.PLAYER, Box.of(it.pos, 200.0, 200.0, 200.0)){ true } } ?: emptyList()

	val receivedSpecies = mutableListOf<Species>()
}