...
open class DNDBipedEntityModel<S: DNDBipedEntityRenderState>(root: ModelPart): EntityModel<S>(root) {
	...
	/**	This function and the 2 after it are direct copies from BipedEntityModel
	 *  [net.minecraft.client.render.entity.model.BipedEntityModel.positionRightArm]
	 */

	private fun positionRightArm(state: S) {
		when (state.rightArmPose) {
			...
			ArmPose.SPEAR -> Lancing.positionArmForSpear(
				this.rightArm,
				this.head,
				true,
				state.getItemStackForArm(Arm.RIGHT),
				state
			)
		}
	}

	private fun positionLeftArm(state: S) {
		when (state.leftArmPose) {
			...
			ArmPose.SPEAR -> Lancing.positionArmForSpear(
				...
				this.head,
				false,
				state.getItemStackForArm(Arm.LEFT),
				state
			)
		}
	}
