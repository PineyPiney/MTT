...
) : CharacterCreatorOptionsTabWidget<Background>(sheet, client, x, y, width, height, message) {
	...
	class BackgroundEntry(background: Background, tab: BackgroundTabWidget, x: Int, y: Int, width: Int, height: Int, message: Text): Entry<Background>(background, tab, x, y, width, height, message){
		...
		override fun render(context: DrawContext, textRenderer: TextRenderer, x: Int, y: Int, scale: Float) {
			...
			context.drawTexture(
				...
				Identifier.of(MTT.MOD_ID, "textures/gui/character_maker/background_icons/${value.id}.png"),
				(x / scale).toInt(),
				(y / scale).toInt(),
				0f,
				0f,
			...
			context.drawText(
				textRenderer,
				Text.translatable("mtt.background.${value.id}"),
				(x / scale).toInt() + 10,
				(y / scale).toInt() + 1,
