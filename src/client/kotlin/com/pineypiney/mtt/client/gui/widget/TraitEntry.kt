...
class TraitEntry(
	...
	val textColour: Int = MTTScreens.textColour
	...
	override fun renderWidget(context: DrawContext, mouseX: Int, mouseY: Int, deltaTicks: Float) {
		...
		context.drawText(tab.client.textRenderer, label, x + 2, y + 2, textColour, false)
		...
		val rotation = DynamicWidgets.easeInOutBack(abs(openness)) * PI.toFloat()
		context.matrices.rotateAbout(rotation, x + width - 6f, y + 6f)
		context.drawTexture(
			RenderPipelines.GUI_TEXTURED,
	...
	override fun onClick(click: Click, doubled: Boolean) {
		...
		if (segment?.onClick(click, doubled, this) == true) return

		val r = x + width
		if (click.x >= r - 12 && click.x < r && click.y >= y && click.y < y + 12) {
	...
	interface Segment{
		...
		fun onClick(click: Click, doubled: Boolean, entry: TraitEntry) = true
	...
	interface TextSegment : Segment{
		...
		fun drawText(
			...
			var lineY = y
			for (line in text) {
				ctx.drawText(textRenderer, line, x, lineY, colour, shadow)
				lineY += 9
			}
		}
	}

	class LiteralTextSegment(val lines: List<OrderedText>, override val textColour: Int = -1) : TextSegment {
		...
		override fun render(
			...
			drawText(ctx, entry.tab.client.textRenderer, lines, x, y, textColour, shadow)
		}
	}

	class OneChoiceSegment<T>(val part: OneChoicePart<T>, val index: Int, override val textColour: Int = -1) :
		TextSegment {
		...
		override fun render(ctx: DrawContext, x: Int, y: Int, mouseX: Int, mouseY: Int, i: Int, entry: TraitEntry, shadow: Boolean) {
			val colour = if (entry.hoveredSegment == i && entry.tab.optionSelectWidget == null) -9985842 else textColour
			val pickText = if(isReady) Text.translatable("mtt.trait.change_answer") else Text.translatable("mtt.trait.pick_a", part.label)
			if (entry.tab.optionSelectWidget == null) pickText.fillStyle(Style.EMPTY.withUnderline(true))
				.fillStyle(Style.EMPTY.withUnderline(true))
			ctx.drawText(entry.tab.client.textRenderer, pickText, x, y, colour, shadow)
			drawText(ctx, entry.tab.client.textRenderer, traitLines, x, y + 9, textColour, shadow)
		}

		override fun onClick(click: Click, doubled: Boolean, entry: TraitEntry): Boolean {
	...
	class GiveNOptsSegment<T>(val part: GivenAndOptionsPart<T>, val index: Int, override val textColour: Int = -1) :
		TextSegment {
		val traitLines  = mutableListOf<OrderedText>()
		override val getNumLines: Int get() = 1 + traitLines.size
		override val isReady: Boolean get() = part.isReady()
	...
	class TallySegment<T>(val part: TallyPart<T>, val index: Int) : Segment {
		...
		override fun render(ctx: DrawContext, x: Int, y: Int, mouseX: Int, mouseY: Int, i: Int, entry: TraitEntry, shadow: Boolean) {
			...
			ctx.drawText(
				textRenderer,
				pointsText,
				x + labelWidth + 5 * part.points + 3 - textRenderer.getWidth(pointsText) / 2,
				y,
				-1,
				shadow
			)
			...
			for((i, value) in part.tallies.withIndex()){
				...
				ctx.drawText(
					textRenderer,
					labels[i],
					x + labelWidth - textRenderer.getWidth(labels[i]),
					buttonY,
					-1,
					shadow
				)
	...
	companion object {
		...
		fun newOf(
			...
			textColour: Int = MTTScreens.textColour
		): TraitEntry {
			val src = when (tab) {
			...
			return newOf(x, y, width, tab, src, label, index, parts, textColour)
		}
		...
		fun newOf(
			...
			val segments: List<Segment> = parts.mapIndexedNotNull { partIndex, part ->
				when (part) {
					is LiteralPart -> LiteralTextSegment(tab.client.textRenderer.wrapLines(part.text, width - 10))
			...
			return TraitEntry(x, y, width, tab, src, label, index, segments, textColour)
		}
	}
}