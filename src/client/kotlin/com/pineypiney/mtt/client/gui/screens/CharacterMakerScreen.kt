...
import com.pineypiney.mtt.client.gui.widget.*
...
class CharacterMakerScreen(handler: CharacterMakerScreenHandler, playerInventory: PlayerInventory, title: Text) : HandledScreen<CharacterMakerScreenHandler>(handler, playerInventory, title) {
	...
	override fun drawBackground(context: DrawContext, deltaTicks: Float, mouseX: Int, mouseY: Int) {
		...
		for(i in 0..3){
			if(i != tabOpen) {
				context.drawTexture(RenderPipelines.GUI_TEXTURED, BACKGROUND, x + 64 * i, y, 192f, 0f, 64, 32, 256, 256)
			}
		}
		context.drawTexture(RenderPipelines.GUI_TEXTURED, BACKGROUND, x, y + 28, 0f, 32f, 256, 224, 256, 256)

		when(tabOpen){
			0 -> context.drawTexture(RenderPipelines.GUI_TEXTURED, BACKGROUND, x, y, 0f, 0f, 64, 32, 256, 256)
			1, 2 -> context.drawTexture(
			...
			3 -> context.drawTexture(RenderPipelines.GUI_TEXTURED, BACKGROUND, x + 192, y, 128f, 0f, 64, 32, 256, 256)
		}
	}
