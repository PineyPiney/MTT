...
class DropDownListWidget<T>(x: Int, y: Int, width: Int, height: Int, val entries: List<T>, val textRenderer: TextRenderer, val getText: (T) -> Text, val entryHeight: Int, message: Text) : ScrollableWidget(x, y, width, height, message) {
	...
	override fun onClick(mouseX: Double, mouseY: Double) {
		if(openButtonHovered(mouseX.toInt(), mouseY.toInt())){
			open = !open
		}
		else if(open){
			val hovered = getHoveredEntry(mouseX.toInt(), mouseY.toInt())
			if(hovered != -1){
	...
	override fun renderWidget(
		...
		if(!open) {
			context.drawTexture(RenderLayer::getGuiTextured, TEXTURE, x, y, 0f, 0f, 50, 12, 64, 16)
			selected?.let {
				context.drawText(textRenderer, getText(it), x + 2, y + 3, -1, false)
			}

			context.drawTexture(RenderLayer::getGuiTextured, TEXTURE, x + 51, y, 52f, 0f, 12, 12, 64, 16)
			if(openButtonHovered(mouseX, mouseY)){
				context.drawBorder(x + 51, y, 12, 12, -1)
			}
		}
		else {
